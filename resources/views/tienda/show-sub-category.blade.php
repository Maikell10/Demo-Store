...
@if (isset($category))
@section('titulo', $sub_category->nombre . ' | ' .$category->nombre.' | '.__('See the Sub-Category').' | TuMiniMercado')
@else
...
<div class="super_container_inner">
    ...
    <div class="products">
        ...
        <div class="container">
            <a href="{{ url('store/show-category/'.$category->slug.'') }}" class="text-success">{{$category->nombre}}</a> <i class="nav-icon fas fa-arrow-right text-warning"></i> <a href="{{ url('store/show-sub-category/'.$sub_category->slug.'') }}" class="text-success">{{$sub_category->nombre}}</a>
        </div>
        
        <div class="container mt-3">

            <h1 class="font-weight-bold"><i class="nav-icon fas fa-shapes text-success"></i> {{__('All Products in')}}
                {{$sub_category->nombre}}</h1>
            ...
            <div class="row row-cols-2 row-cols-md-4">
                ...
                <div class="col mb-4">
                    ...
                    <div class="card-footer">
                        <small class="text-muted">{{__('See all in')}} <a href="{{ url('store/show-main-category/'.$producto->main_category->slug.'') }}"
                                class="text-success">{{$producto->main_category->nombre}}</a></small>
                    </div>
